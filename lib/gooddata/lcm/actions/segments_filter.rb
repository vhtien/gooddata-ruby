    ...
    class SegmentsFilter < BaseAction
      ...
      class << self
        def call(params)
          ...
          downcase_segment_ids = filtered_segments.map(&:segment_id).map(&:downcase)
          duplicated_segment_ids = downcase_segment_ids.select { |e| downcase_segment_ids.count(e) > 1 }.uniq
