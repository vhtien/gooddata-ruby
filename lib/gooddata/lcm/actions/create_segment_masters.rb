    ...
    class CreateSegmentMasters < BaseAction
      ...
      class << self
        def call(params)
          ...
          # Concurrent::Promise.zip(*synchronize_projects).value!

          # loop do
          #   break if synchronize_projects.all?(&:complete?)
          #   sleep 0.5
          # end
          ...
          # Return results
          {
            results: results,
            params: {
              # synchronize: synchronize_projects.map(&:value)
              synchronize: Concurrent::Promise.zip(*synchronize_projects).value!
            }
          }
        end
